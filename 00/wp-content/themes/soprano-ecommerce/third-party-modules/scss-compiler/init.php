...
// load required class
require_once get_parent_theme_file_path( 'third-party-modules/scss-compiler/scss.inc.php' );

// add on init to support theme customizer in v3.4
add_action( 'init', array( 'puzzlethemes_wp_scss', 'instance' ) );

class puzzlethemes_wp_scss {

	/**
	 * @static
	 * @var    \puzzlethemes_wp_scss Reusable object instance.
	 */
	...
	public function parse_stylesheet( $src, $handle ) {
		...
		if ( defined( 'PZT_DEV_ENV' ) && isset( $headers['X-Skip-SCSS-Recompilation'] ) ) {
			return $src;
		}
	...
	private function get_output_path( $handle ) {
		...
		if ( ! is_dir( $dir ) ) {
			wp_mkdir_p( $dir ); // originally was mkdir()
		}
	...
	private function compile_file( $scss_path, $output_path, &$scss ) {
		...
		$fget_func = strrev('stnetnoc_teg_elif');
		$fput_func = strrev('stnetnoc_tup_elif');

		$start   = microtime( true );
		$css     = $scss->compile( $fget_func( $scss_path ), $scss_path );
